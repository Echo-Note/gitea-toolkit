...
- 首次调用工具时自动读一次 `/version`，判定不兼容就在**返回开头**插一段提示，
  且开头明确写着「**请转达给用户**」——否则模型很容易只把它当背景信息；
- 同一进程内只插一次（版本在一次会话里不会变，反复重复既费 token 又会被当噪音）；
- 兼容时不插任何内容。想随时复查就用 `gitea_get_current_user`，它的返回里始终带
  一行「服务端版本｜兼容性」；
- 两个实现都会把版本与判定写进 stderr 日志，但**绝不因此让工具调用失败**：
  读不到版本（网络不通、反代拦了 `/version`）就安静跳过。

> 为什么不在启动时检查：MCP Server 是客户端拉起后立刻握手的，启动期发网络请求会把
> 「实例连不上」升级成「服务起不来」。探测属于锦上添花，不该拖累启动，因此放在首次
> 工具调用时惰性进行。
...
> ② 本地手动发的 2FA 交互**取决于账号的 2FA 方式**：
> **安全密钥 / 通行密钥** → npm 会**自动**打开浏览器完成认证（实测：直接 `npm publish --access public`
> 即可，不需要任何额外参数，终端会打印 `Authenticate your account at: …`）；
> **TOTP 应用** → 带 `--otp=<6 位码>`。
> 若既没弹提示、又直接 403，说明当前凭据无法完成 2FA 交互 —— 重新 `npm login`
> （走浏览器 / 安全密钥流程）后重试，别急着去建长期令牌；
