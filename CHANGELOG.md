...
- **「工作流定义」一栏永远是空的，而「最近运行」里明明有记录。**
  ...
  现在**接口返回为空时回落到直接列工作流文件**（`.gitea/workflows` 与
  `.github/workflows` 都列，按目录顺序去重）。实测同两个仓库能列出 **10 / 13 个**。

  代价是拿不到启用状态，所以这类节点渲染为**状态未知**：右侧标出所在目录
  （如 `.github/workflows`），图标不再用「禁止符」—— 不能凭空断言它被停用了。
